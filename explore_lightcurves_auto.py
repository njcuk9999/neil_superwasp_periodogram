...
DPATH = WORKSPACE + '/Data/selected_lightcurve_data_redo_auto.fits'
...
def plot_bounds(s_id, x, y, ey):
    plt.close()
    # Do plot
    fig, frame = plt.subplots(ncols=1, nrows=1)
    # plot all points in black
    frame.errorbar(x, y, yerr=ey, ls='none', marker='o', color='k',
                   markersize=2)
    # plot sub regions
    groupmasks = pf2.subregion_mask(x, MINPOINTS, MAXGAP)
    colours = ['r', 'b', 'g', 'm', 'c', 'y'] * len(groupmasks)
    for g, m in enumerate(groupmasks):
        frame.errorbar(x[m], y[m], yerr=ey[m], ls='none', marker='o',
                       color=colours[g], markersize=2)
...
if __name__ == "__main__":
    ...
    for sid in tqdm(do_sids):
        ...
        groupbounds = pf2.subregion_bounds(time, MINPOINTS, MAXGAP)
        if PLOT:
            plot_bounds(sid, time, flux, eflux)
        # Deal with data acquisition
        if maxsegments < len(groupbounds):
            maxsegments = len(groupbounds)
        for groupbound in groupbounds:
            starts, ends = sort_data_in_dict(sid, starts, ends, groupbound[0],
                                             groupbound[1])
