...
try:
    ...
    raise ModuleNotFoundError("clean_periodogram.py needed")
try:
    from neil_clean import neil_clean
except ModuleNotFoundError:
    raise ModuleNotFoundError(" Neail_clean.py needed")
try:
...
NYQUIST_FACTOR = 100
FAP_LEVELS = [0.01]
PEAK_RANGE = [0.1*3600*24, 100.0*3600*24]
...
FREQS = 1.0/np.arange(0.00199, 714.232118, 0.0042753)
...
def messina_plot(time, data, edata, name, lfreq, lpower, mlfreq, mlpower,
    ...
    if (np.sum(np.isfinite(lpower)) > 0) and (np.sum(lmask) > 0):
        ...
        if (np.sum(np.isfinite(mlpower)) > 0) and (np.sum(mlmask) > 0):
    ...
    if (np.sum(np.isfinite(cpower)) > 0) and (np.sum(cmask) > 0):
        ...
        # normed_mcpower = np.max(normed_cpower)*normed_mcpower/np.max(
        #                           normed_mcpower)
    ...
    do_table = True
    for pp in ps:
        if np.sum(np.isfinite(pp)) == 0:
            do_table = False
    for npp in nps:
        if np.sum(np.isfinite(npp)) == 0:
            do_table = False
    if do_table:
...
if __name__ == "__main__":
    ...
    # load data from file
    if '.fits' in pp['DPATH']:
        lightcurve = fits.getdata(pp['DPATH'])
    ...
    print('\n Running Monte Carlo on Lomb Scargle...')
    ...
    if pp['MESSINA_PERIOD'] is not None:
        ...
        goodpeaks = []
        goodpeaklabels = []
    # check for values (i.e. not all nan/inf)
    do_table = True
    for pps in ps:
        if np.sum(np.isfinite(pps)) == 0:
            do_table = False
    for npps in nps:
        if np.sum(np.isfinite(npps)) == 0:
            do_table = False
    # select good and bad peaks from matching periods
    if do_table:
        spercentage = pp['PEAK_THRESHOLD_PERCENTAGE']
        sresults = select_matching_periods(ps, nps, percentage=spercentage)
